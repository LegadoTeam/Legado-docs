...
| `purchaseChapter(chapterUrl, chapter)` | 购买 VIP / 付费章节 | ⚡ 可选  |
...
| 类型 | 英文标识 | 元数据标记                      | `chapterContent` 返回值     |
| ---- | -------- | ------------------------------- | --------------------------- |
| 小说 | `novel`  | `// @type novel` 或省略（默认） | 纯文本字符串                |
| 漫画 | `comic`  | `// @type comic`                | `JSON.stringify(imageUrls)` |
| 视频 | `video`  | `// @type video`                | 播放地址（URL 或 JSON）     |
| 音乐 | `music`  | `// @type music`                | 音频播放地址（URL 或 JSON） |

未标记 `@type` 或标记为 `@type novel` 时，均视为**小说书源**（默认类型）。
