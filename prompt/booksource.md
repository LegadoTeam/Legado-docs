...
::: tip 使用方法
推荐先下载 [Legado-docs 仓库](https://github.com/LegadoTeam/Legado-docs)，再让 AI 引用仓库中的 `public-docs/prompt/booksource.md` 作为起始提示词。若所用工具无法读取本地文件，也可以将下方提示词整体复制为系统提示词（System Prompt）。
:::
...
- `category === 'GETALL'` 或未命中任何分类时，可返回分类名字符串数组；有同名分类时返回 `{ name, url }[]`，前端显示 `name`、请求传 `url`
- 分类对象可带 `style.layout_flexGrow` 与 `style.layout_flexBasisPercent`，移动端会按旧阅读 FlexBox 宽度提示展示，桌面端暂不使用
...
- **必须正序返回**（第一章在前）
- VIP / 付费章节标记为 `vip: true`，可附加 `price` 和 `currency`
- URL 去重、过滤无关链接
...
### 模块 C+：purchaseChapter（VIP 章节购买，可选）

仅当目录中存在 `vip: true` 且站点提供购买接口时实现。不要在 `chapterList()` 或 `chapterContent()` 中自动购买。

```js
async function purchaseChapter(chapterUrl, chapter) {
  var resp = await legado.http.post(
    "https://example.com/api/buy",
    JSON.stringify({ url: chapterUrl }),
    {
...
购买成功后应用会自动重试 `chapterContent()`。

> 📖 详见：[guide/purchase-chapter](../guide/purchase-chapter.md) · [api/types-purchase-result](../api/types-purchase-result.md)
...
async function chapterContent(chapterUrl) {
  ...
  // 去噪声
  text = text.replace(
    /本章未完|加入书签|章节报错|请收藏|最快更新|天才一秒记住/g,
    "",
  );
  return text.trim();
}
...
async function chapterContent(chapterUrl) {
  ...
  if (!imgs.length)
    imgs = legado.dom.selectAllAttrs(doc, ".comic-page img", "data-src");
...
m3u8Text = m3u8Text.replace(/^(?!#)(.+\.ts.*)$/gm, base + "$1");
return m3u8Text;
...
```js
// 一次性执行
var result = legado.browser.run(url, "return document.title", {
  visible: false,
});
...
```js
{ name, url, group?, vip?, price?, currency? }   // group 仅视频多线路需要；vip=true 表示付费章
```
