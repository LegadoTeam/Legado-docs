...
- `BookItem` 字段：`{ name, author, bookUrl, coverUrl, kind, lastChapter }`
- 站点不支持发现页时直接跳过
...
**实现要点**：
- 返回 `{ name, author, coverUrl, intro, kind, lastChapter, tocUrl }`
...
```js
{ name, author, bookUrl, coverUrl, kind, lastChapter }
```
